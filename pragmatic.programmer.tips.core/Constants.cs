...
internal static class Constants
{
    ...
    public const string LogRegex =
        @"\[@(?<datetimestamp>[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{1,3})?Z) \| (?<loglevel>[a-zA-Z]+)]";

    public const string ThisDirectory = "./";
    public const string ColonDelimiter = ":";
    public const string LogLevelRegexGroupKey = "loglevel";

    public const string CertificateCouldNotBeConvertedIntoX509Certificate2 =
        "The certificate could not be converted into a X509Certificate2, could not continue";
    ...
    public const string ResetTipIdentifierRemembranceFile =
        "reset tip identifier text file using _tipsRepository.DeleteTipIdentifierTextFile()";

    public const string StoredTipIdentifierInRemembranceFile =
        "stored tip identifier using _tipsRepository.WriteTipIdentifiersToTextFile(...)";
